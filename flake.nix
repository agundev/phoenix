        ...
        nixpkgs.lib.extend (
          final: prev:
          {
            ...
            recursiveConcatMapAttrs =
              f: v: prev.foldl' prev.recursiveUpdate { } (prev.attrValues (prev.mapAttrs f v));
          }
          // home-manager.lib
        );
      lib = mkLib nixpkgs;
    ...
    flake-parts.lib.mkFlake { inherit inputs; } (
      { withSystem, ... }:
      {
        ...
        flake = lib.recursiveConcatMapAttrs (
          ...
          let
              ...
              let
                deepMerge =
                  ...
                  if builtins.isList lhs && builtins.isList rhs then
                    ...
                    lib.mapAttrs (k: _: deepMerge (lhs.${k} or null) (rhs.${k} or null)) (lhs // rhs)
            ...
            deployPkgs = import nixpkgs {
              ...
              overlays = [
                (
                  final: prev:
                  lib.recursiveUpdate (deploy-rs.overlays.default final prev) {
                    deploy-rs.deploy-rs = prev.deploy-rs;
                  }
                )
              ];
            };
          in
          rec {
            nixosConfigurations.${hostname} = withSystem config.hardware.system (
              { self', inputs', ... }:
            ...
            deploy.nodes.${hostname} = withSystem config.hardware.system (
              { self', inputs', ... }:
              {
                inherit hostname;
                    ...
                    value = {
                      ...
                      path =
                        deployPkgs.deploy-rs.lib.activate.home-manager
                          self'.legacyPackages.homeConfigurations.${home};
                    };
                  }) config.homes
                ));
              }
